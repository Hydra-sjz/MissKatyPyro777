...
from anilist.data_parser import search_filler, parse_filler
...
@anibot.on_message(
    filters.command(['anlfillers', f"anlfillers{BOT_NAME}"], prefixes=trg)
)
@control_user
async def anl_fillers_cmd(client: anibot, message: Message, mdata: dict):
...
async def fillebrs_cmd(client: anibot, message: Message):
