...
MUSIC_MAX_LENGTH = 3600 #10800
DELAY_DELETE_INFORM = 40
TG_THUMB_MAX_LENGTH = 320
REGEX_SITES = (
    r"^((?:https?:)?\/\/)"
    r"?((?:www|m)\.)"
    r"?((?:youtube\.com|youtu\.be|soundcloud\.com|mixcloud\.com|on.soundcloud.com))" # 
    r"(\/)([-a-zA-Z0-9()@:%_\+.~#?&//=]*)([\w\-]+)(\S+)?$"
)
REGEX_EXCLUDE_URL = (
...
async def _fetch_and_send_music(bot, message: Message):
    ...
    try:
        ...
        if info_dict['duration'] > MUSIC_MAX_LENGTH:
            ...
            await _reply_and_delete_later(message, inform,
                                          DELAY_DELETE_INFORM)
            return
        d_status = await message.reply_text("__⏳ Your request is processing...__", quote=True,
...
async def _upload_audio(message: Message, info_dict, audio_file):
    ...
    caption = f"<i><a href=\"{webpage_url}\">song.link</a> | <a href=\"http://t.me/GojoSatoru_Xbot\">via</a></i>"
    duration = int(float(info_dict['duration']))
    performer = info_dict['uploader']
    songcop = await message.reply_photo(photo=f"{squarethumb_file}", caption=f"🎧 <b>Title:</b> {title}\n🔗 <b>Link:</b> <a href=\"{webpage_url}\">Click here</a>\n❗️<b>Is Local:</b> False")
